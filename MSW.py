...
def format_text_to_send(web_str,number_of_days):
    days = ["Today", "Tomorrow", "2d from now","3d from now","4d from now","5d from now","6d from now"]
...
def main():
    ...
    number_of_days = 3
    message_to_print,placeholder_flag = format_text_to_send(text,number_of_days)
    if placeholder_flag != (48 * number_of_days):
